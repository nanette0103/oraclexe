...
DDL(data Definition Language) - 데이터 정의어
    데이터 베이스 내의 구조 및 객체(테이블, 시퀀스 등)을 생성, 변경, 삭제하기 위해 사용되는 SQL문입니다. 
*/
/*
CREATE TABLE문
    데이터를 저장할 테이블을 생성합니다.
...
CREATE TABEL dept3 AS SELECT * FROM dept WHERE 1=2;

/*
ALTER문
    데이터베이스 객체 구조나 속성을 변경할 때 사용되는 SQL명령문
*/
...
제약조건(constraint)
    ...
    테이블의 생성 또는 변경시 설정하는 조건입니다.(지정한 데이터의 신뢰성 높이기 위함)
    ...
    FOREIGN KEY(외래키, 참조키)
        부모 조건의 PRIMARY KEY를 참조하는 칼럼에 붙이는 제약조건입니다.
